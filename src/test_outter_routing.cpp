#include <topology.h>
// 兩個問題：fingerring 有可能不完整，第二net ordering 順序要相反
...
//          pckg.generate_case_using_vias();
    ...
    auto    sol2    = planar_orderd_forest_solution<>( roots, outer_bumps, outer_nets );

    sol1.find_MST_by_comparable_length();
    sol2.find_MST_by_comparable_length();
    ...
    auto out = std::ofstream( "x.txt" );
    cf2.txt_vitualizer2( out );
    out.close();
